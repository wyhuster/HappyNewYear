...
public class MyRadioGroup extends LinearLayout {
	...
	public List<RadioButton> findRadioButton(ViewGroup group) {
		List<RadioButton> resBtns = new ArrayList<RadioButton>();
		int len = group.getChildCount();
		for (int i = 0; i < len; i++) {
			if (group.getChildAt(i) instanceof RadioButton) {
				RadioButton resBtn = (RadioButton) group.getChildAt(i);
				resBtns.add(resBtn);
			} else if (group.getChildAt(i) instanceof ViewGroup) {
				resBtns.addAll(findRadioButton((ViewGroup) group.getChildAt(i)));
			}
		}
		return resBtns;
	}
		...
		public void onChildViewRemoved(View parent, View child) {
			if (parent == MyRadioGroup.this && child instanceof RadioButton) {
				((RadioButton) child).setOnCheckedChangeListener(null);
					...
					&& child instanceof ViewGroup) {
				List<RadioButton> buttons = findRadioButton((ViewGroup) child);
				for (RadioButton button : buttons) {
